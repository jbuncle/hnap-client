...
public class DeviceSettingsParserTest extends TestCase {
    ...
    public void testCreateFromResponse() throws Exception {
        ...
        final String xml = IOUtils.toString(
                DeviceSettingsParserTest.class.getClassLoader()
                        .getResourceAsStream("uk/co/jbuncle/hnapclient/response/DeviceSettings.xml")
        );

        final Map<String, Object> responseProperties = XmlToObject.fromXml(xml);

        final DeviceSettingsI result = DeviceSettingsParser.createFromResponse(responseProperties);
        ...
        expectedSOAPActions.add(new URL("http://purenetworks.com/HNAP1/SetFactoryDefault"));
